...
import { reactive, computed, useTemplateRef, ref, watch, nextTick } from "vue";
import {
  message,
  Drawer,
  Badge,
...
import { use_config_store, is_script_play_mode } from '@/store/config';
...
/** 宽窄屏切换时禁用布局过渡，避免歌词页弹出/翻页特效 */
const layout_transition_disabled = ref(false);
watch(is_desktop, () => {
  layout_transition_disabled.value = true;
  nextTick(() => {
    requestAnimationFrame(() => {
      layout_transition_disabled.value = false;
    });
  });
});
...
const { sync_shortcuts_with_play_mode } = use_global_shortcuts();

const music_filter_placeholder = computed(() =>
  is_script_play_mode(config_store.play_mode)
    ? 'F1开始自动演奏，F2停止演奏'
    : '聆听模式仅可播放；可在设置中切换为「演奏」',
);

watch(
  () => config_store.play_mode,
  (mode) => {
    void sync_shortcuts_with_play_mode(mode);
  },
  { immediate: true },
);
...
use_app_bootstrap(init_music_list, init_app_info);
  ...
  <ConfigProvider :locale="zhCN">
    ...
    <div
      ...
      :class="{
        ...
        'no-layout-transition': layout_transition_disabled,
      }"
    >
      <div
      ...
      >
        <div class="settings-open">
          ...
          <Input
            ...
            :placeholder="music_filter_placeholder"
...
@media (max-width: @desktop-breakpoint) {
  ...
  .main-layout.no-layout-transition .list-face,
  .main-layout.no-layout-transition .lyrics-face {
